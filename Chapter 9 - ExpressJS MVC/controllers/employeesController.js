...
const deleteEmployee = (req, res) => {
	...
	if (!employee) {
		return res
			.status(400)
			.json({ message: `Error: Cannot find employee id ${req.body.id}` });
	}
	const updatedData = data.employees.filter((emp) => emp.id != req.body.id);
...
const getEmployee = (req, res) => {
	...
	if (!employee)
		return res
			.status(400)
			.json({ message: `Error: Employee id ${req.params.id}} not found.` });
